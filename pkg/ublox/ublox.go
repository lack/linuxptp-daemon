...
func (u *UBlox) UbloxPollInit() {
	...
	if err != nil {
		...
		// There is no reader to complete when the command could not be
		// started, but close its per-reader completion channel so stop/reset
		// can use the same lifecycle path.
		close(doneCh)
		...
		go func() {
			defer close(doneCh)
			u.ubloxPollPushThread(reader, stopCh)
		}()
	}
}
